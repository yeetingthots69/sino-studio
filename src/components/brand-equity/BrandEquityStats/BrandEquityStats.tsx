...
export default function BrandEquityStats() {
    const dict = useDictionary().brandEquity.stats;
    const ref = useRef<HTMLDivElement>(null);
    const inView = useInView(ref, {once: true, amount: 0.4});

    const stats = Object.keys(dict).map((key) => ({
        key,
        value: dict[key as keyof typeof dict].value,
        suffix: dict[key as keyof typeof dict].suffix,
        label: dict[key as keyof typeof dict].label,
    })) as Stat[];
    ...
    return (
        <section className={styles.section} ref={ref}>
            ...
            >
                {stats.map((stat, i) => {
                    return <StatItem key={stat.key} stat={stat} index={i} trigger={inView}/>;
                })}
            </motion.div>
        </section>
    );
